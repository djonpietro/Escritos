...
Uma transformação de uma variável aleatória $X$ com imagem $\mathcal{X}$ é toda função
...
então $f(X) = f \circ X$ é uma variável aleatória se, e somente se,
\end{def:variavel derivada}

\begin{prop:prob transformacao}
\label{prop:prob transformacao}
Seja $X$ uma variável aleatória, $f$ uma transformação e $A \in \borel$.
\[
...
Seja $X$ uma variável discreta com suporte $\Omega_X$, FMP igual a $p_X$,
$f$ uma transformação de $X$ e $Y = f(X)$.
A variável $Y$ é discreta e sua FMP é dada por
\[
...
\begin{proof}
	...
	\[
		P(Y \in A) > 0 \Longleftrightarrow P(X \in f^{-1}(A)) > 0,
	\]
	...
	O resultado sobre a FMP de $Y$ segue do fato de que
	\[
		p_Y(y) = P(Y = y) = P(X \in f^{-1}(y))
	\]
	e temos que
	\[
		p_Y(y) = \sum_{x \in f^{-1}(y)} p_x(X)
...
\begin{proof}
	...
	\]
	Sendo $g$ monótona crescente crescente, $g^{-1}$ também será, de modo que
	\[
