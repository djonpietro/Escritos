...
conjunto menor de resultados possíveis. Outra interpretação possível é aquela tal que
associamos uma probabilidade a um evento tendo o conhecimento de uma evidência sobre ele.
...
Se $E$ e $F$ são eventos independentes de $\Omega$, então
\[
...
Seja $\Omega$ um espaço amostral e $\mathcal{F} = \{F_1, \ldots, F_n\}$ uma partição de
$\Omega$. Se $E$ é um evento de $\Omega$, então
\[
	P(E) = \sum_{F \in \mathcal{F}} P(E) \cdot P(E | F)
\]
\begin{proof}
	Para quaisquer $i, j \in [n]$ temos que
	\[
