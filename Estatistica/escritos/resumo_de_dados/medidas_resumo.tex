...
Uma família de transformações comuns para um variável $X$ são as de Turkey:
\[
  T(X, p) = \begin{cases}
    x^p,&\text{ se } p > 0\\
    \log x,&\text{ se } p = 0 \\
...
o valor de $p$ a fim de corrigi-la. .Se a distribuição for levemente
assimétrica à direita, valores transformações com $ 0 < p < 1$ provocarão uma
correção leve em valores grandes. Transformações como a logaritmíca são
adequadas para assimetrias à direita moderadas, especialmente quando o desvio
padrão é proporcional à média, ou quando o efeito modelado é multiplicativo
invés de aditivo. Já transformações com $p < 0$ são aplicadas em assimetrias à
direita severas, em que o sinal negativo nos valores pode ou não ser usado para
preservar a sua ordem. Para assimetrias à esquerda, valores de $p > 1$ provocam
uma compressão de valores pequenos e expandem os valores maiores.
