...
\begin{def:combinacao}
  ...
  de tamanho $r$ é uma função $f: A \to \mathbb{N}$ que satisfaz
\end{def:combinacao}
...
\begin{thm:enum combinacao}
 ...
 \begin{proof}
   ...
   subconjuntos de $r$ elementos que podem ser obtidos de $A$. Suponha que haja
   ...
   Pela função de contagem de arranjos, temos então:
   \begin{gather}
      x \cdot r! = \frac{n!}{(n-r)!} \\
      x = \frac{n!}{(n-r)!r!}
   \end{gather}
...
é como ordenações de elementos de $A$ tal que cada elemento repita-se
tantas vezes quanto forem a sua multiplicidade.
...
\begin{def:permutacao com repeticao}
  ...
  e que satisfaz
  \[
    \forall a \in A,\ C = \{x \in [T] \mid f(x) = a\} \longrightarrow |C| = m(a)
  \]
...
a multiplicidade desse elemento.
...
\begin{thm:enum permutacao com repeticao}
  ...
  \begin{proof}
    ...
    iremos contar as permutações com repetições as igualando ao número de tuplas
