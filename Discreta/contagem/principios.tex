...
o problema fundamental de cada princípio e demonstrar o seu funcinamento.
...
\begin{thm:pfc}
  ...
  finitos. A cardinalidade do produto cartesiano $R$ desses conjuntos
  é igual ao produto das cardinalides deles.
  \[
    |R| = \prod_{A \in \mathcal{F}} |A|
  \]
  \begin{proof}
    A prova será por indução no número de conjuntos $n$ em $\mathcal{F}$.

    Base: Se $n = 2$, então suponha que $\mathcal{F}$ seja formada
    por dois conjuntos $A_1 = A$ e $A_2 = B$. Admita também
    uma partição de $R$ dada por $\mathcal{P}$ tal que
    \[
      \mathcal{P} = \{P_1, \ldots, P_m\}
    \]
    em que, para $k \in [m]$, tem-se
    \[
      P_k = \{(a_k, b) \mid b \in B\}
    \]
    e $a_k$ é o k-ésimo elemento numa ordenação para $A$. Considere agora
    uma família de funções indexadas por $[m]$ tal que
    \begin{gather*}
      f_k: B \to P_k \\
      f_k(b) = (a_k, b)
    \end{gather*}
    ...
    $\mathcal{P}$, temos que a cardinalidade de $R$ é igual a
    \[
    ...
    conjuntos, para qual o teorema vale. Temos então
    \[
